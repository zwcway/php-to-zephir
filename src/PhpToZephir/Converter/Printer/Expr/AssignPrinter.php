...
class AssignPrinter
{
    ...
    public function convert(Expr\Assign $node, $extract = true)
    {
        ...
        if ($rightNode instanceof Expr\Array_) {
            ...
            $vars = [$this->dispatcher->pPrec($leftNode, $precedence, $associativity, -1)];
    ...
    {
        if (property_exists($node,
                'left') === true && ($node->left instanceof Expr\MethodCall || $node->left instanceof Expr\FuncCall)
        ) {
        ...
        if (property_exists($node,
                'right') === true && ($node->right instanceof Expr\MethodCall || $node->right instanceof Expr\FuncCall)
        ) {
        ...
        if (property_exists($node,
                'expr') === true && ($node->expr instanceof Expr\MethodCall || $node->expr instanceof Expr\FuncCall)
        ) {
        ...
        return ['extracted' => $collected, 'node' => $node];
    }
    ...
    private function convertListStmtToAssign($node)
    {
        ...
        $listVarName = str_replace(['[', ']', '"'], '', $listVarName);

        $pList[] = 'let ' . $listVarName . ' = ' . $this->dispatcher->pPrec($rightNode, $precedence, $associativity,
                1) . '';
     ...
     * @param Assign $rightNode
     */
    private function findVarToAssign($rightNode, array $toAssign = [])
    {
        if ($rightNode->expr instanceof Expr\Assign) {
     ...
     */
    private function arrayDimFetchCase(
        $node,
        $leftNode,
        $rightNode,
        $operatorString,
